SELECT user.name, 
       ...
       commit.message,
FROM   {{ ref('staging_commit') }} AS `commit` 
JOIN   {{ ref('staging_user_email') }} AS `user_email` ON user_email.email = commit.committer_email 
JOIN   {{ ref('staging_user') }} AS `user` ON user_email.user_id = user.id